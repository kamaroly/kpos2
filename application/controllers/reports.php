...
{	
	...
	*/
	...
	public function smsreport(){
        ...
        if(strlen($destination)<12 || strlen($destination)<24 ){
        ...
        $alldestination=explode(',', $destination);
        foreach ($alldestination as $key => $MSISDN) {
        //Formatting URL to send the SMS
        $url = "http://{$server}/bulksms/bulksms?username={$username}&password={$password}&type=0&dlr=1&destination={$MSISDN}&source=kamaro&message={$message}";
