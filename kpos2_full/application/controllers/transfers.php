...
class Transfers extends Secure_area
{
	...
	function change_locations()
	{
	    ...
	    $this->sale_lib->set_from_location($this->input->post('from_location_id'));
	    $this->sale_lib->set_to_location($this->input->post('to_location_id'));
	...
	
   {
	...
	if (array_key_exists( $this->lang->line('sales_credit'), $data['payments']) AND $customer_id==-1)
	 ...
	 {//Looks normal so far...
	   ...
	   $cust_info=$this->Customer->get_info(null);
       ...
       $data['amount_change']=to_currency($this->sale_lib->get_amount_due() * -1);
		...
		$data['sale_number']=$this->Transfer->save($data['cart'],null,$employee_id,$comment,$data['payments'],$sale_id=false,$total,null,null,null);
		...
		{
			 // HMVC example
             $this->load->controller('sales/complete', $params = array('transfers'=>TRUE), $return = FALSE);
			//save the sale per currency
			//$this->Transfer->save_currency_sales($data['sale_number'],$data['subtotal'],$total);
    	...
    	if ($this->sale_lib->get_email_receipt() && !empty($cust_info->email))
			{
				...
				//$this->email->message($this->load->view("transfers/receipt_email",$data, true));	
		...
		//Show the receipt
		//$this->load->view("transfers/receipt_non_printing",$data);
	...
	
{
	...
	function _reload($data=array())
	{
		...
		// TRANSACTION TYPE
	    $data['transaction_types']=array('S'  =>$this->lang->line('transfers_transfer'),
	    	                             'R'  =>$this->lang->line('transfers_return'));

	
        //Check if the users is allowed to be in many locations
        if($this->employee_location==0):
	    $data['from_locations']= model\Locations_model::all();
	    $data['to_locations']= model\Locations_model::all();
	    else:
	    $data['to_locations']= model\Locations_model::all();
	    $data['from_locations']= model\Locations_model::find($this->employee_location);	
        endif;
        //Get the set location
        $data['from_location']=$this->sale_lib->get_from_location();
        $data['to_location']  =$this->sale_lib->get_to_location();
