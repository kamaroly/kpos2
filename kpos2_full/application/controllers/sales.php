...
class Sales extends Secure_area
{
	...
	//Attempt file write
	//-------------------
     if ( ! write_file("$mypath$receipt_file_name.txt",$sales_data,"wd"))
       {
  ...
  function add($item_id=-1)
	 ...
	 */
 function complete($transfers=FALSE)
	...
	if (array_key_exists( $this->lang->line('sales_credit'), $data['payments']) AND $customer_id==-1)
	 ...
	 {//Looks normal so far...
       ...
       $data['amount_change']=to_currency($this->sale_lib->get_amount_due() * -1);
		...
		$data['sale_number']=$this->Sale->save($data['cart'], $customer_id,$employee_id,$comment,$data['payments'],$sale_id=false,$total,null,null,null);
