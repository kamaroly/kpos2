...
class Receivings extends Secure_area
{
	...
	function test($transfer_id)
	{
		$data=$this->transfer->get_transfer_items($transfer_id)->result_array();
		var_dump($data);

	}
	function add($transfer_id=null)
	{
		$data=array();
		...
		elseif($this->receiving_lib->is_valid_transfer($transfer_id))
		{
			$this->receiving_lib->receiving_entire_transfer($transfer_id);
		}
	 ...
	 * @name transfers()
	 * @method to display all transfers sales
	 * 
	 */
	function transfers()
	{
		$data = array();
		$data['receiving_transfers'] = $this->transfer->get_all()->result_array();
		$this->load->view('transfers/transfers', $data);
	}
