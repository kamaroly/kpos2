...
class Employees extends Person_controller
{
	...
	*/
	function view($employee_id=-1)
	{
        ...
        $data['locations']= model\Locations_model::all();
	...
	*/
	function save($employee_id=-1)
	{
		...
		$locations_data  = $this->input->post('store_list')!=false  ? $this->input->post("store_list"):array();
		...
		if($this->input->post('password')!='')
		{
			...
			'location_id' => intval($locations_data [0])
			);
		}
		...
		if($this->Employee->save($person_data,$employee_data,$permission_data,$employee_id,$locations_data))
		{
			//New employee
			if($employee_id==-1)
			{
