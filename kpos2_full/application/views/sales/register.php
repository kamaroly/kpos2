...
if (isset($success))
{
	...
	if($success==$this->lang->line('sales_transaction_success') || $success==$this->lang->line('sales_successfully_saved_quotation'))
	{
		?> 
...
$(window).load(function()
{
 <?php if($this->config->item('print_receipt_size')=='receipt_tape'):?>
	 window.open('<?php echo site_url('sales/certifiedinvoiceprinting/');?>',"receipt_window","toolbar=no, location=no, directories=no, status=yes, menubar=no, scrollbars=yes, resizable=1, copyhistory=no, width=330, height=700"); 
 <?php else:?>
     window.open('<?php echo site_url('sales/certifiedinvoiceprinting/');?>',"receipt_window","toolbar=no, location=no, directories=no, status=yes, menubar=no, scrollbars=yes, resizable=1, copyhistory=no,width=980, height=700"); 
 <?php endif;?>
...
<?php echo form_dropdown('receipt_type',$receipt_types,substr($mode,0,1),'onchange="$(\'#mode_form\').submit();"'); ?>

<?php echo form_dropdown('transaction_type',$transaction_types,substr($mode,1,1),'onchange="$(\'#mode_form\').submit();"'); ?>
...
<div id="overall_sale">
	...
	<?php echo $current_payment_currency?$current_payment_currency_info->Symbol.' '.$amount_due/$current_payment_currency_info->Exchange_Rate:to_currency($amount_due); ?>
		...
		if(substr(strtolower($mode),0,1)=='p'):
	     
	     echo form_open("sales/save_quotation",array("id"=>"finish_sale_form")); 
         echo "<button class='btn btn-large btn-success' id='finish_sale_button' 
		             style='margin-top:5px;width:99%'>
		                <span>".$this->lang->line('sales_complete_proforma')."</span>
		              </button>";
		echo form_close();
