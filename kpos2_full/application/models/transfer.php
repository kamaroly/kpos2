...
class Transfer extends CI_Model
{
	public function get_info($sale_id)
	{
		...
		$this->db->where('sale_id',$sale_id);
		 //check if employee is limited to one location
        if($this->employee_location!=0)
        {
		$this->db->where('transfers.location_id',$this->employee_location);
        }
		return $this->db->get();
	}
   ...
   function get_all()
	...
	function update($sale_data, $sale_id)
	{
		...
		$this->db->where('sale_id', $sale_id);
		$success = $this->db->update('transfers',$sale_data);
	...
	function check_sale_exits($sale_id)
	{
	  ...
	  return $this->db->get('transfers')->num_rows();
	...
	function get_max_sale_id()
	{
		...
		return $this->db->get('transfers')->row()->sale_id+1;
	}
/**
 * @Author Kamaro Lambert
 * @param  array $items
 ...
 function save ($items,$customer_id,$employee_id,$comment,$payments,$sale_id=false,$total,$invoice_number=null,$date_issued=null,$po_number=null,$sdc_information=null)
	{
		...
		$sales_data = array(
			...
			'from_location_id'    => $this->sale_lib->get_from_location(),
			'to_location_id'      => $this->sale_lib->get_to_location(),
		);
		...
		IF($sale_id!=false)
		{
			...
			$this->db->where('sale_id', $sale_id);
			$this->db->update('transfers', $sales_data);
			$sale_id = $sale_id;
			
			foreach($payments as $payment_id=>$payment)
			{
				if ( substr( $payment['payment_type'], 0, strlen( $this->lang->line('transfers_giftcard') ) ) == $this->lang->line('transfers_giftcard') )
				{
				...
				if ( substr( $payment['payment_type'], 0, strlen( $this->lang->line('transfers_credit') ) ) == $this->lang->line('transfers_credit') )
						
				{
					$sales_payments_data['credit']=1;
				}
				$this->db->where('sale_id', $sale_id);
				$this->db->update('transfers_payments',$sales_payments_data);
			}
			...
			foreach($items as $line=>$item)
			{
				...
				$this->db->update('transfers_items',$sales_items_data);
				...
				if ($customer_id == -1 or $customer->taxable)
				{
					...
					{
						$this->db->where('sale_id', $sale_id);
						$this->db->update('transfers_items_taxes', array(
								'sale_id' 	=>$sale_id,
								'item_id' 	=>$item['item_id'],
								'line'      =>$item['line'],
								'name'		=>$row['name'],
		...
		else 
		{
			...
			//It's a new saled
		$this->db->insert('transfers',$sales_data);
		...
		foreach($payments as $payment_id=>$payment)
		{
			...
			$this->db->insert('transfers_payments',$sales_payments_data);
		}
		...
		foreach($items as $line=>$item)
		{
			...
			$this->db->insert('transfers_items',$sales_items_data);
			...
			$customer = $this->Customer->get_info($customer_id);
 			if ($customer_id == -1 or $customer->taxable)
 			{
				foreach($this->Item_taxes->get_info($item['item_id']) as $row)
				{
					$this->db->insert('transfers_items_taxes', array(
						'sale_id' 	=>$sale_id,
						'item_id' 	=>$item['item_id'],
						'line'      =>$item['line'],
						'name'		=>$row['name'],
	...
	function delete($sale_id)
	{
		...
		$this->db->delete('transfers_payments', array('sale_id' => $sale_id)); 
		$this->db->delete('transfers_items_taxes', array('sale_id' => $sale_id)); 
		$this->db->delete('transfers_items', array('sale_id' => $sale_id)); 
			 //check if employee is limited to one location
        if($this->employee_location!=0)
        {
		$this->db->where('transfers.location_id',$this->employee_location);
        }
		$this->db->delete('transfers', array('sale_id' => $sale_id)); 
	...
	function get_transfer_items($sale_id)
	{
		$this->db->from('transfers_items');
        $this->db->join('transfers', $this->db->dbprefix('transfers_items').'.sale_id='.$this->db->dbprefix('transfers').'.sale_id');
		$this->db->where($this->db->dbprefix('transfers').'.sale_id',$sale_id);
        //check if employee is limited to one location
        if($this->employee_location!=0)
        {
		 $this->db->where('transfers.location_id',$this->employee_location);
	...
	{
		$this->db->from('transfers_payments');
		$this->db->where('sale_id',$sale_id);
		return $this->db->get();
	}
	...
	public function create_sales_items_temp_table($StartDAte=null,$EndDate=null,$sale_id=null)
	{
		...
		if($this->employee_location!=0)
        {
		 $location_condition=' AND '.$this->db->dbprefix('transfers').'.location_id='.$this->employee_location;
        }


		$this->db->query("DROP TABLE IF EXISTS ".$this->db->dbprefix('transfers_items_temp'));
		$this->db->query("CREATE TEMPORARY TABLE ".$this->db->dbprefix('transfers_items_temp')."
		(SELECT date(sale_time) as sale_date, ".$this->db->dbprefix('transfers_items').".sale_id, comment,payment_type, customer_id, 
		...
		".$this->db->dbprefix('transfers_items').".line as line, serialnumber, ".$this->db->dbprefix('transfers_items').".description as description,
		...
		FROM ".$this->db->dbprefix('transfers_items')."
		INNER JOIN ".$this->db->dbprefix('transfers')." ON  ".$this->db->dbprefix('transfers_items').'.sale_id='.$this->db->dbprefix('transfers').'.sale_id'."
		$period_condition 	$sale_id_condition	$location_condition
		INNER JOIN ".$this->db->dbprefix('items')." ON  ".$this->db->dbprefix('transfers_items').'.item_id='.$this->db->dbprefix('items').'.item_id'."
		LEFT OUTER JOIN ".$this->db->dbprefix('suppliers')." ON  ".$this->db->dbprefix('items').'.supplier_id='.$this->db->dbprefix('suppliers').'.person_id'."
		LEFT JOIN ".$this->db->dbprefix('people')." ON ".$this->db->dbprefix('people').".person_id=".$this->db->dbprefix('transfers').".customer_id
		LEFT OUTER JOIN ".$this->db->dbprefix('transfers_items_taxes')." ON  "
		.$this->db->dbprefix('transfers_items').'.sale_id='.$this->db->dbprefix('transfers_items_taxes').'.sale_id'." and "
		.$this->db->dbprefix('transfers_items').'.item_id='.$this->db->dbprefix('transfers_items_taxes').'.item_id'." and "
		.$this->db->dbprefix('transfers_items').'.line='.$this->db->dbprefix('transfers_items_taxes').'.line'."
			
		GROUP BY sale_id, item_id, line)");

		//Update null item_tax_percents to be 0 instead of null
		$this->db->where('item_tax_percent IS NULL');
		$this->db->update('transfers_items_temp', array('item_tax_percent' => 0));

		//Update null tax to be 0 instead of null
		$this->db->where('tax IS NULL');
		$this->db->update('transfers_items_temp', array('tax' => 0));

		//Update null subtotals to be equal to the total as these don't have tax
		$this->db->query('UPDATE '.$this->db->dbprefix('transfers_items_temp'). ' SET total=subtotal WHERE total IS NULL');
	}
	...
	function set_receipt_copied($sale_id)
	{
		       ...
		       $this->db->where('sale_id', $sale_id);
		return $this->db->update('transfers',array('copied' => TRUE,
