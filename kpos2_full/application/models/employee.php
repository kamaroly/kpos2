...
class Employee extends Person
{
	...
	*/
	function save(&$person_data, &$employee_data,&$permission_data,$employee_id=false,$locations_data)
	{
		$success=false;
		...
		if(parent::save($person_data,$employee_id))
		{
			...
			if($success)
			{
				...
				//now insert employee locations
				if($success)
				{
					foreach($locations_data as $allowed_location)
					{
						//if we don't have location then update it
						if(!$this->has_location($allowed_location,$employee_id))
...
/*
	Determins whether the employee specified employee has access the specific module.
	*/
	function has_location($location_id,$person_id)
	{
		//if no module_id is null, allow access
		if($location_id==null)
		{
		...
		$query = $this->db->get_where('employees_locations', array('employee_id' => $person_id,'location_id'=>$location_id), 1);
		return $query->num_rows() == 1;
