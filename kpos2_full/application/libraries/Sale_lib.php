...
class Sale_lib
{
  ...
  /**
   * [set_receipt_label description]
   * @param [type] $receipt_label [description]
   */
	function set_receipt_label($receipt_label)
	{
		$this->CI->session->set_userdata('rlabel',$receipt_label);
	}
    
    /**
     * [set_sdc_receipt_data description]
     * @param [type] $receipt_data [description]
     */
    function set_sdc_receipt_data($receipt_data)
    {
    	$this->CI->session->set_userdata('receipt_data',$receipt_data);
    }
     ...
     * @param [type] $receipt_data [description]
     */
    function get_sdc_receipt_data()
    {
    	return $this->CI->session->userdata('receipt_data');
    }

    /**
     * [clear_po_number description]
     * @return [type] [description]
     */
    function clear_receipt_data()
	{
		$this->CI->session->unset_userdata('receipt_data');
	}
	...
	function set_from_location($location_id)
	{
		$this->CI->session->set_userdata('from_location',$location_id);
	}
	/**
	 * @author Kamaro Lambert
	 * @method to get the current PO number which is in the session
	...
	{
		return $this->CI->session->userdata('from_location');
	}
	 ...
	 * @example sale_lib->clear_po_number();
	 */
	function clear_from_location()
	{
		$this->CI->session->unset_userdata('from_location');
	}

		function set_to_location($location_id)
	{
		$this->CI->session->set_userdata('to_location',$location_id);
	}
	/**
	 * @author Kamaro Lambert
	 * @method to get the current PO number which is in the session
	...
	{
		return $this->CI->session->userdata('to_location');
	}
	 ...
	 * @example sale_lib->clear_po_number();
	 */
	function clear_to_location()
	{
		$this->CI->session->unset_userdata('to_location');
	}
	...
	function set_sale_sdc_error($sale_sdc_error)
	{
		$this->CI->session->set_userdata('sale_sdc_error',$sale_sdc_error);
	}
	
	function get_sale_sdc_error()
	{
		return $this->CI->session->userdata('sale_sdc_error');
	}
	
	function clear_sale_sdc_error()
	{
		return $this->CI->session->unset_userdata('sale_sdc_error');
	}

	function get_mode($person_info=null)
	{
		if(!$this->CI->session->userdata('sale_mode'))
		{
			 if(!is_null($person_info) AND $person_info->working_mode=='training')
			 {
                $this->set_mode('TS');
			 }
			 else
	...
	function copy_entire_sale($sale_id)
	{
		...
		$this->set_sdc_receipt_data($this->CI->Sale->get_info($sale_id)->row()->sdc_information);
	...
	function clear_all()
	{
		...
		$this->clear_sale_id();
		$this->clear_receipt_data();

		$this->clear_sale_sdc_error();
    ...
    /**
     * method to convert tab into html spaces
     * @param  string  $line_or_string [description]
     * @param  integer $tab  [description]
     * @param  boolean $nbsp [description]
     * @return string        [description]
     */
	function tab2space($line_or_string, $tab = 4, $nbsp = FALSE) 
	{
